...
class GMailMessage(object):
        ...
        for json_object in json_objects:
            if "text_attachment_data" in json_object:
                self.content += self._gmail_decode_string(
                    json_object["text_attachment_data"]
                )
            elif (
                "body" in json_object["payload"]
                and "data" in json_object["payload"]["body"]
            ):
                self.content += self._gmail_decode_string(
                    json_object["payload"]["body"]["data"]
                )
    ...
    def _gmail_decode_string(self, string):
        return base64.b64decode(string.replace("-", "+").replace("_", "/")).decode(
            "utf-8"
        )
    ...
    def markAsRead(self):
        if not self.isRead():
            ...
            callAPI(
                self.corpus.service.users()
                .messages()
                .modify(userId=self.corpus.userID, id=self.id, body=resource)
            )
    ...
    def moveToTrash(self):
        if not self.isTrash():
            callAPI(
                self.corpus.service.users()
                .messages()
                .trash(userId=self.corpus.userID, id=self.id)
            )
    ...
    def removeFromTrash(self):
        if self.isTrash():
            callAPI(
                self.corpus.service.users()
                .messages()
                .untrash(userId=self.corpus.userID, id=self.id)
            )
    ...
    def stringify(self):
        return '[{}] {} -> <"{}", {}>'.format(
            self.date, self.sender_email, self.subject, self.labels
        )
